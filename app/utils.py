...
#         """Find window geometry using wmctrl and xwininfo"""
#         try:
#             # Get window ID using wmctrl
#             output = subprocess.check_output(['wmctrl', '-l'], timeout=2).decode()
...
#             # Get window geometry with xwininfo (including frame decorations)
#             xwininfo = subprocess.check_output(
#                 ['xwininfo', '-id', window_id, '-frame'], timeout=2).decode()

#             # Parse geometry from xwininfo output
#             geometry = {
#                 'x': int(re.search(r'Absolute upper-left X:\s+(\d+)', xwininfo).group(1)),
#                 'y': int(re.search(r'Absolute upper-left Y:\s+(\d+)', xwininfo).group(1)),
...
#             primary_match = re.search(r'primary (\d+)x(\d+)\+(\d+)\+(\d+)', screen_info)
#             if primary_match:
#                 primary_x = int(primary_match.group(3))
#                 primary_y = int(primary_match.group(4))
#                 geometry['x'] -= primary_x
#                 geometry['y'] -= primary_y
...
#         """Start capturing the application window"""
#         self.geometry = self._find_window_geometry()
#         if not self.geometry:
#             raise RuntimeError(f"Could not find window for {self.app_name}")

#         # Setup MSS monitor configuration
#         self.monitor = {
...
#         with mss.mss() as sct:
#             last_valid_geometry = self.monitor.copy()
...
#                     print(f"Capture error: {e}. Attempting to reacquire window...")
#                     # Try to find window again
#                     new_geometry = self._find_window_geometry()
#                     if new_geometry:
#                         self.monitor = {
#                             "left": new_geometry['x'],
#                             "top": new_geometry['y'],
...
#                         last_valid_geometry = self.monitor.copy()
#                     else:
#                         self.monitor = last_valid_geometry
...
#         self.system = platform.system()
#         self.geometry = None
#         self.monitor = None  # Added for MSS compatibility
...
#     def _capture_loop(self):
#         """Main capture loop using MSS with precise window capture"""
#         with mss.mss() as sct:
#             # Get the exact screen coordinates for MSS
#             print("Available monitors:", sct.monitors)  # Add this for debugging
#             self._calculate_mss_monitor(sct)
#             print("Using monitor:", self.monitor)
...
#                     self._reacquire_geometry(sct)
#                 time.sleep(0.033)

#     def _calculate_mss_monitor(self, sct):
#         """Convert window geometry to MSS-compatible monitor coordinates"""
#         if not self.geometry:
#             return

#         # Get all monitors information using MSS correctly
#         monitors = sct.monitors[1:]  # Skip first (all-in-one) monitor
        
#         # Find which monitor contains our window
#         for monitor in monitors:
#             mo_left = monitor["left"]
#             mo_top = monitor["top"]
#             mo_right = mo_left + monitor["width"]
#             mo_bottom = mo_top + monitor["height"]
            
#             # Check if window is within this monitor
#             if (self.geometry['left'] >= mo_left and 
#                 self.geometry['top'] >= mo_top and
#                 (self.geometry['left'] + self.geometry['width']) <= mo_right and
#                 (self.geometry['top'] + self.geometry['height']) <= mo_bottom):
                
#                 # Convert to monitor-relative coordinates
#                 self.monitor = {
#                     "left": self.geometry['left'] - mo_left,
#                     "top": self.geometry['top'] - mo_top,
#                     "width": self.geometry['width'],
#                     "height": self.geometry['height']
#                 }
#                 return

#         # Fallback to absolute coordinates if not found
#         self.monitor = {
...
#     def _reacquire_geometry(self, sct):
#         """Reacquire window position if capture fails"""
#         print("Reacquiring window geometry...")
#         self.geometry = self.find_window_geometry()
#         self._calculate_mss_monitor(sct)
...
#                 title = win32gui.GetWindowText(hwnd)
#                 if self.app_name.lower() in title.lower():
#                     # Get client rect (excluding window decorations)
...
#         windows = []
#         win32gui.EnumWindows(callback, windows)
#         # ... rest of Windows implementation remains same ...
...
#             # Get precise geometry including frame
#             xwininfo = subprocess.check_output(
#                 ['xwininfo', '-id', window_id, '-frame'], timeout=5).decode()

#             # Parse geometry from xwininfo output
#             geometry = {
#                 'left': int(re.search(r'Absolute upper-left X:\s+(\d+)', xwininfo).group(1)),
#                 'top': int(re.search(r'Absolute upper-left Y:\s+(\d+)', xwininfo).group(1)),
#                 'width': int(re.search(r'Width:\s+(\d+)', xwininfo).group(1)),
#                 'height': int(re.search(r'Height:\s+(\d+)', xwininfo).group(1))
#             }

#             # Adjust for window decorations
#             geometry['left'] += int(re.search(r'Relative upper-left X:\s+(-?\d+)', xwininfo).group(1))
#             geometry['top'] += int(re.search(r'Relative upper-left Y:\s+(-?\d+)', xwininfo).group(1))
